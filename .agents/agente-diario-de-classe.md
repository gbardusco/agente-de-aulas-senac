...
1. Receba o **relato livre** do professor. Consulte `AULAS/aula-XX/index.md`, slides/fonte, rubrica aprovada e `AULAS/registros-docentes/aula-XX/relatorio.json`, se existirem. Não peça para o professor escrever os próprios campos finais.
2. Redija imediatamente os campos sustentados pelos fatos disponíveis. Separe as lacunas em `pendencias`, com perguntas específicas; não bloqueie textos possíveis por faltar feedback individual ou recuperação. Planejado não significa aplicado.
...
4. Escreva primeiro `AULAS/registros-docentes/aula-XX/redacao.json`: `{ "aulaId": "aula-XX", "campos": [{ "id": "atividades-chamada", "valor": "texto redigido com base no relato" }], "pendencias": ["pergunta específica"], "evidencias": [], "fontes": [], "historico": [] }`. Use os IDs da configuração do diário; os metadados completos seguem `_templates/relatorio-docente.schema.json`. Execute `npm run relatorio -- gerar --aula aula-XX --valores AULAS/registros-docentes/aula-XX/redacao.json` (adicione `--config` se existir configuração institucional). O comando valida e renderiza HTML/JSON, não redige nem chama IA. Preserve e incorpore os valores, evidências e histórico anteriores; só use `--sobrescrever` para a atualização solicitada. `--modelo` é exclusivo para pedido explícito de formulário vazio, nunca uma entrega de relatório. Use os assets do setup. Duplique grupos com IDs únicos somente quando necessário.
